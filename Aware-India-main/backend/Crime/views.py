...
@api_view(['GET'])
def home(request):
    # NOTE: Dont use this till production Remember Limited Call present 
    # api = NewsDataApiClient(apikey=NewsDataApiClient)
    # response = api.news_api(category='crime' ,country = "in" , language='en') 
    ...
    total=Crime_report.objects.count()
...
def State_crime(request,state):
    crimes = Crime_report.objects.filter(state=state).values('state').annotate(crime_count=Count('report_number')).order_by('-date_of_occurrence')
    total=crimes.count()
    return Response({'crime':crimes.values('date_reported','date_of_occurrence','city','crime_description','victim_age','crime_domain')})
